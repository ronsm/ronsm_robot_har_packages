...
class Main():
    ...
    def create_restricted_train_dbs(self):
        path = self.rel_path + '/src/DBs/' + self.mln_prefix + '_DB_h.txt'
        restricted_train_db_h = Database(self.mln_h, dbfile=path)
        restricted_train_db_h.tofile(path)

        path = self.rel_path + '/src/DBs/' + self.mln_prefix + '_DB_s.txt'
        restricted_train_db_s = Database(self.mln_s, dbfile=path)
        restricted_train_db_s.tofile(path)
    ...
    def load_restricted_train_dbs(self):
        path = self.rel_path + '/src/DBs/' + self.mln_prefix + '_DB_h.txt'
        self.restricted_train_db_h = Database.load(self.mln_h, dbfiles=path)

        path = self.rel_path + '/src/DBs/' + self.mln_prefix + '_DB_s.txt'
        self.restricted_train_db_s = Database.load(self.mln_s, dbfiles=path)
    ...
    def ros_new_mln_callback(self, msg):
        log = 'Received request to create new MLNs: ' + msg.data
        self.logger.log(log)
        name = msg.data
    ...
    def ros_load_mln_callback(self, msg):
        log  = 'Received request to load MLNs: ' + msg.data
        self.logger.log(log)
        name = msg.data
    ...
    def ros_mln_train_callback(self, msg):
        mode = msg.data
        log = 'Changing MLN training mode to: ' + mode
        self.logger.log(log)
        if mode == 'global':
            self.train_mode = 'global'
            ...
            self.logger.log('Invalid MLN train mode specified.')
    ...
    def save_evidence(self, label_h, label_s):
        ...
        if self.mln_prefix != 'default':
            path_h = self.rel_path + '/src/DBs/' + self.mln_prefix + '_DB_h.txt'
            file = open(path_h, 'a')
            ...
            path_s = self.rel_path + '/src/DBs/' + self.mln_prefix + '_DB_s.txt'
            file = open(path_s, 'a')
